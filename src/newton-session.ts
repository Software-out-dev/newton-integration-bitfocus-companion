...
import { DEFAULT_CONFIG, type ModuleConfig } from './config.js'
import { SETTINGS } from './settings.js'
import { gainKey } from './feedbacks.js'
...
import { arraysEqual, createInitialNewtonState } from './state.js'
import {
	parseClockSelected,
...
import type { GainReadState, NewtonState, SPRResponse, SnapshotInfo } from './protocol/types.js'

// Every feedback that renders H2L priority state and must refresh together.
export const PRIORITY_FEEDBACK_IDS = ['input_patch_monitor'] as const
// Feedbacks that render processing-clock state.
export const CLOCK_FEEDBACK_IDS = ['clock_monitor'] as const

/**
 * The narrow contract the session uses to reach Companion. The session never
 * sees the instance, its definitions or its per-control maps: it reports
 * through these callbacks and the instance decides how to publish.
 */
export interface SessionHost {
	log(level: LogLevel, message: string): void
	updateStatus(status: InstanceStatus, message?: string): void
	/** Republish the device variables derived from the session state. */
	updateVariables(): void
	checkFeedbacks(...feedbackIds: string[]): void
	/** Throttled per-channel VU publish. */
	updateVuVariables(): void
	/** Immediate VU publish that also flushes a pending throttled one. */
	publishVuVariablesNow(): void
	/** Re-register definitions (e.g. the snapshot-by-name dropdown changed). */
...
/**
 * Owns the device session: the TCP command client, the UDP meter listener,
 * every poll timer and retry gate, the session metadata and the polled
 * NewtonState. Companion-facing registration and publishing stay in the
 * instance adapter, reached only through the SessionHost callbacks.
 */
export class NewtonSession {
	private tcpClient: NewtonTcpClient | null = null
	...
	// update this object first, then the host publishes the small subset
	// Companion needs.
	private state: NewtonState = createInitialNewtonState()

	constructor(private readonly companion: SessionHost) {}

	/** The session always works on an already-normalized config copy. */
	configure(config: ModuleConfig): void {
		this.config = config
	}

	/** State is replaced on device reset: always read it through this getter. */
	getState(): NewtonState {
		return this.state
	}

	/** The current TCP client, for binding action definitions to one session. */
	getClient(): NewtonTcpClient | null {
		return this.tcpClient
	}

	isConnected(): boolean {
		return this.tcpClient?.isConnected ?? false
	}
	...
	/** Final teardown when the Companion instance is destroyed. */
	destroy(): void {
		this.destroyed = true
		this.stop()
	}
	...
	private isCurrentClient(client: NewtonTcpClient): boolean {
		return !this.destroyed && this.tcpClient === client
	}
	...
	connectToDevice(): void {
		...
		this.companion.updateStatus(InstanceStatus.Connecting)
		...
		client.on('disconnected', () => {
			...
			this.companion.log('warn', 'Disconnected from Newton device')
		})

		client.on('error', (err) => {
			if (!this.isCurrentClient(client)) return
			this.companion.log('error', `Connection error: ${err.message}`)
			this.state.connected = false
			this.clearPriorityState()
			this.companion.updateStatus(InstanceStatus.ConnectionFailure, err.message)
			this.companion.updateVariables()
			this.companion.checkFeedbacks(
				'connection_status',
				'connection_monitor',
	...
	private async readFirmwareVersion(client: NewtonTcpClient | null = this.tcpClient): Promise<boolean> {
	...
	private markSnapshotsUnsupported(reason: string): void {
		...
		this.companion.log('warn', reason)
	...
	private requestSnapshotDatabase(client: NewtonTcpClient | null = this.tcpClient): void {
		if (this.state.snapshotsUnsupported) return
		if (!client?.isConnected) return
		client
	...
	private async pollPresetAudio(): Promise<void> {
		const client = this.tcpClient
		if (!client?.isConnected || !this.companion.hasGainSubscribers()) return
		...
		try {
			...
			if (recovery.recovered) this.companion.log('info', 'Preset-audio polling recovered')
			let changed = false
			...
			if (changed) this.companion.checkFeedbacks('channel_gain', 'channel_mute')
		} catch (err) {
	...
	private registerPresetAudioFailure(attempt: number, reason: string): void {
		...
		if (failure.enteredBackoff) {
			...
			this.state.gainReads.clear()
			this.companion.checkFeedbacks('channel_gain', 'channel_mute')
			this.companion.log('warn', `Preset-audio polling temporarily unavailable; retrying with backoff: ${reason}`)
		}
	}
	...
	private async pollNextPriorityList(client: NewtonTcpClient | null = this.tcpClient): Promise<void> {
		...
		try {
			...
			if (parsed) {
				...
				this.companion.updateVariables()
				if (changed) {
					this.companion.checkFeedbacks(...PRIORITY_FEEDBACK_IDS)
				}
			}
		} catch (err) {
			if (!this.isCurrentClient(client)) return
		...
		try {
			...
			if (parsed) {
				...
				if (!prev || prev.list[0] !== parsed.list[0] || prev.isForced !== parsed.isForced) {
					this.companion.checkFeedbacks(...CLOCK_FEEDBACK_IDS)
				}
			}
		} catch (err) {
			if (!this.isCurrentClient(client)) return
	...
	startVuListener(): void {
		...
		listener.on('vuLevels', (levels) => {
			...
			if (this.udpStreamLost) {
				this.udpStreamLost = false
				this.companion.log('info', `UDP status stream from ${this.config.host}:${SETTINGS.vuPort} recovered`)
			}
		...
		listener.start()
		this.companion.log('info', `VU UDP polling started at ${this.config.host}:${SETTINGS.vuPort}`)
	}
	...
	private clearUdpPriorityState(): void {
		...
		this.state.clockSelected = new Array(CLOCK_TYPE_COUNT).fill(-1)
		this.companion.updateVariables()
		this.companion.checkFeedbacks(...PRIORITY_FEEDBACK_IDS, ...CLOCK_FEEDBACK_IDS)
	}
