...
Gain/mute actions accept at most 32 active or waiting operations per connection. All actions have a 4.5-second completion budget, including queue waits and device reads, so they return before Companion's 5-second action timeout. Excess or expired requests report an action error. After expiry, queued commands are discarded and unfinished reads cannot trigger a later write. A command already sent cannot be undone: its outcome is unconfirmed at expiry, and it keeps its normal response timeout to preserve the TCP response order. A late reply does not replace the action error with success. Disconnecting or changing the target also cancels waiting gain/mute requests and prevents unfinished reads from sending a later write; new requests must be made after reconnection.
